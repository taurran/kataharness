...
- **`run-closed` is terminal and written exactly once**, by `close_run`. Nothing is legal on the
  cursor after it; `run_start`'s resume test reads it (`kata_dispatch.is_run_closed`).
