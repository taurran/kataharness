...
| `livenessDeadline` | minutes (int) | F3 dark-worker detection (a **dispatch/execution** concern, NOT sprint/delivery — read by the sprint-blind orchestrator's liveness monitor): a worker with a `CLAIM` but no `DONE`/`PROGRESS` for longer than this is *stale* → nudge → escalate → human-gated re-dispatch (never a blind kill). Default `10`; absent ⇒ 10 min (BC). |
