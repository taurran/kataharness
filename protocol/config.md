...
| `modules` | `string[]` | Active à-la-carte modules beyond the mode's bundle (D20): e.g. `["slop","debug","benchmark"]` — every entry must name a provider-backed module (a skill tagged `kata/module/<name>`; fail-closed load-guard, D45) — see the *Optional modules* registry below for the documented optional set. |
...
| `roles` | `{ "<role>": { platform: string, model?: string, effort?: string } }` | **Multi-model routing (D105, spec multi-model-orchestration) — wired**: resolved + validated **fail-closed at preflight** via `kata_roles.resolve_roles` — unknown role / unconfirmed platform / **a host-only role (`orchestrator`\|`evaluator`, `kata_roles.HOST_ONLY_ROLES`, LD11) routed off-host** ⇒ **STOP + escalate** (the host-only constraint is code-enforced, not advisory). Routes a loop **role** (`coder`\|`validator`\|`researcher`\|`orchestrator`\|`evaluator`\|`design-author`\|`plan-author`) to a platform/model. **Absent ⇒ every role on the host platform (BC1, single-model — today's behavior).** A role assigned to a non-host platform must name a platform in `.kata-settings.json` `confirmedPlatforms`. **`design-author`/`plan-author`** (KH-T13, dispatch-authoring spec) are the two freeze-stage authoring roles — neither is host-only (`kata_roles.HOST_ONLY_ROLES` unchanged); their normalized payload shapes (`designPath`/`planPath` + `verdict` + `deviations`) are defined by `tools/kata_dispatch.py`'s `normalize()`, not restated here. |
...
| `cursor` | `{ pushTrail: bool }` | Cursor/trail **durability preferences** (trust-model DESIGN §2.5, TM-C3). **`cursor.pushTrail`** is the REMEMBERED PREFERENCE for the trail-push **OFFER** closeout presents at the human push gate (Decision 2, alongside commit/push/merge) — it is presented *as* the resilience option, never taken on the operator's behalf. **Default never-push:** absent config, absent `cursor` block, and absent `pushTrail` key all mean `false` — today's behavior byte-for-byte (BC1). It **authorises nothing on its own**: `tools/kata_trail.py` contains no push path at all, and the flag is **structurally excluded from the resilience level** — `kata_trail.derive_resilience` grades `Verified (full)` only on a push **RECEIPT recorded on the cursor**, never on this flag (DESIGN §6.2's "never the config flag", enforced in code rather than stated). Read by `kata_trail.read_push_trail`; **present-but-malformed** (a non-dict `cursor` block, a non-bool `pushTrail`) ⇒ **RAISE** — load-guard STOP, never a silent coercion to the default (D45/GB12, the house rule). Needs no `kata_config.validate_core_config` entry (additive keys are legal by that validator's own contract). Erratum of record: per-run trail refs live at **`refs/kata/trails/<runId>`** (`kata_trail.RUN_TRAIL_REF_PREFIX`) — the DESIGN's `refs/kata/trail/<runId>` is unconstructible while the legacy `refs/kata/trail` ref exists (a git directory/file ref conflict); the legacy ref is untouched. |
...
**Anchor and critical work (R7):** Zero-step critical work (advanced-critical + standard-critical; resolved rung `== anchor_index`) resolves to `None`/OMIT regardless — it **needs no anchor detection** and inherits by omission (always current, immune to a gated top rung and immune to a stale anchor). Only **below-anchor cells** read the anchor name to compute a rung: economy tier-down (`standard-coding`, `standard-economy`, `advanced-economy` *(Anthropic, `−1`)*, all `essential` work). An **unknown anchor** (id not found in the family ladder) → `resolve()` returns `None` → inherit — never a crash, never a forced model. **⚠ Qualified by the T-11 anchor load-guard:** `resolve()`'s inherit-on-doubt contract is unchanged, but `kata_models.validate_anchor` — called at the `kata-orchestrate` precondition load-guard — **RAISES** for the narrow case of a `claude-<word>-<digit…>` anchor naming no known rung (a vendor rename / unmodelled tier), because that anchor silently costs BOTH economy tier-down and the advisor rung with nothing surfaced. A ladder short-name, a vendor id containing a known rung in any hyphen position, a Bedrock/Vertex-prefixed carrier of the same id, the `"session"` sentinel, a foreign-family id, and un-versioned product names all pass. So: **never a crash at RESOLVE time; a deliberate loud STOP at LOAD time for that one shape.**
...
- **Core load-guard mechanism (GB12/D45):** a PRESENT config's `mode` / `tiers` / `modules` are validated fail-closed by `kata_config.validate_core_config(config, available_skills, provided_modules)` (`tools/kata_config.py`) at the `kata-orchestrate` precondition — a `mode` outside `essential|standard|advanced`, a `tiers[family]` naming no `kata-<family>-<tier>` skill (`tiers["kata-grill"] == "skip"` excepted, D71/D73), or a `modules[]` entry with no provider skill (no skill tagged `kata/module/<module>`; full `kata/module/<x>` and short `<x>` entry forms both resolve to the tag) ⇒ RAISE ⇒ STOP + escalate. Absent keys pass (documented defaults apply, D25). `effort.reasoning` is deliberately not strictly validated (indicative, not an API contract — see the `effort` row above); malformed JSON is caught at the read step, before validation. Input sets derive from `validate_skills.load_skills()` via `kata_config.available_from_skills`.
