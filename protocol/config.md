...
| `roles` | `{ "<role>": { platform: string, model?: string, effort?: string } }` | **Multi-model routing (D105, spec multi-model-orchestration) — DESIGN-STAGED, proof-slice only; NOT yet read by `kata-orchestrate`'s load-guard.** Routes a loop **role** (`coder`\|`validator`\|`researcher`\|`orchestrator`\|`evaluator`) to a platform/model. **Absent ⇒ every role on the host platform (BC1, single-model — today's behavior).** A role assigned to a non-host platform must name a platform in `.kata-settings.json` `confirmedPlatforms`. The resolver `tools/kata_roles.py` + dispatch `tools/kata_dispatch.py` exist + are tested, but **wiring into the orchestrator load-guard + dispatch path is PARKED** (HANDOFF NEXT). A hand-edited `roles` block is NOT validated at preflight until that wiring lands. |
