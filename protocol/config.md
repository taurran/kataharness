...
| `roles` | `{ "<role>": { platform: string, model?: string, effort?: string } }` | **Multi-model routing (D105, spec multi-model-orchestration).** Routes a loop **role** (`coder`\|`validator`\|`researcher`\|`orchestrator`\|`evaluator`) to a platform/model. **Absent ⇒ every role on the host platform (BC1, single-model — today's behavior).** A role assigned to a non-host platform must name a platform in `.kata-settings.json` `confirmedPlatforms` (else the fail-closed load-guard STOPs at preflight). Resolved by `tools/kata_roles.py`; dispatched via `tools/kata_dispatch.py` (a NEW task-brief + per-platform headless-CLI adapter). |
