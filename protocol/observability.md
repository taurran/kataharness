...
| Checkpoint trailers | `Kata-Checkpoint: {...}` trailer in commit messages on the task's **active attempt branch** (`<task>` or `<task>-attemptN`) | Worker, via `kata_telemetry emit-trailer` CLI (kata_telemetry.py:1016-1066) | **Committed** as part of normal task-branch history — durable as long as the branch/ref survives |
...
- **v1 vs v2 vs v3:** an absent `v` reads as v1 (the pre-schema shape). v2 is additive: `perTask`
  ...
  **v3 (v0.2.1, CA-L40) is additive over v2:** `build_ledger_row` now emits `v: 3` with one new column,
  ...
  (kata_telemetry.py:539-544); only absent-`v` silently maps to v1. **No backfill** — row 1 (v1) stands
...
- **Cost columns are honest, not complete.** `tokensIn`/`tokensOut` (row-level and per-task) are nullable
  — host-dependent, the usage-meter honesty (kata_telemetry.py:748-749, 873-876). A `null` means "not
...
- **Merge commits are never a trailer source.** `scan_checkpoints` walks the branch's own non-merge
  ...
  multi-parent commit), a trailer-free merge is silently skipped — not an error (kata_telemetry.py:227-265).
