...
|---|---|---|---|---|---|---|
| `kata-advise` | 0.1.1 | 3 | plan | experimental | new (KataHarness original, advisor-executor initiative — GRILL-LEDGER S-1/S-2/S-7/S-24, DESIGN §3.8/§5). Fresh-context no-write consult pattern (L4/L5) applied to a scoped Fable-tier advisor; conductor-dispatched only, mirroring kata-research's escalation-routed no-write dispatch (D62). | Fable-tier advisory consult on an intensive issue — diagnosis / approach / risks for the redispatch brief; advisory, never gates |
...
| `kata-plan-advanced` | 0.3.1 | 4 | plan | beta | adapted-from mattpocock/skills {to-issues vertical-slicing} + GSD plan-phase + BMAD {trade-offs-over-verdicts} + CPP plan format | Dispatched as role `plan-author` (KH-T13) once DESIGN.md is frozen, gated by `protocol/authored-artifact-gate.md` before entering the main tree |
| `kata-plan-essential` | 0.3.1 | 2 | plan | beta | adapted-from mattpocock/skills {to-issues vertical-slicing} + GSD plan-phase + BMAD {trade-offs-over-verdicts} + CPP plan format | Dispatched as role `plan-author` (KH-T13) once DESIGN.md is frozen, gated by `protocol/authored-artifact-gate.md` before entering the main tree |
| `kata-plan-standard` | 0.3.1 | 3 | plan | beta | adapted-from mattpocock/skills {to-issues vertical-slicing} + GSD plan-phase + BMAD {trade-offs-over-verdicts} + CPP plan format | Dispatched as role `plan-author` (KH-T13) once DESIGN.md is frozen, gated by `protocol/authored-artifact-gate.md` before entering the main tree |
...
| `kata-tdd` | 0.4.1 | 3 | execute | beta | adapted-from mattpocock/skills engineering/tdd | Red-green-refactor on a vertical slice |
