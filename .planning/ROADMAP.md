...
| Milestone 2 — Freeze/Float (D138) | **IN PROGRESS (operator-directed)** — ship order M1→M4→M2→M3. **M1 COMPLETE + MERGED** (PR #5 -> master `0c82bc4`, 2026-07-02): P0 engine + P1 substrate + D139 integrated adval + **P2 THE FLOAT** (triple-freeze-gated, D140; `contract_gate.py` + orchestrate 0.5.0 + evaluate 0.3.0 + RUBRIC surfaces; BC — dormant until a plan declares `builds_against`). **M4 (inline evaluator/reroll) is NEXT** per ship order. |
