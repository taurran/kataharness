...
modified — **AMENDED (escalation E-T2-1, conductor decision 2026-07-14): ONE named exception.**
T2's worker correctly escalated that `test_install_cli_headless.py::test_no_flag_stdout_golden`
byte-pins the vaultless plain-install output — exactly the surface frozen S2 deliberately
changes (stdout field + stderr note). A golden pinning superseded behavior would fail the
design; the golden is UPDATED (option a) to byte-pin the DESIGNED post-S2 output, keeping it a
strict BC anchor of the new contract. All other existing tests stay untouched.
