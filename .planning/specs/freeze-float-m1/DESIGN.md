...
status: revised-post-freeze-gate
created: 2026-07-02
revised: 2026-07-02
baseline_branch: hardening/kenjiri-lessons
freeze_gate: v1 HOLD (7 findings) → all folded → re-gate pending
tags:
  - kata/freeze-float
  - kata/spine
  - contract-edges
...
`invalidation_set` risked a silent-permissive empty set. Every cited reuse point verified in-code. The
reshaped design below folds all seven.
...
- **M1-L3 — Invalidation is durable + independently re-derived + fail-closed (companion 1; F3/F5/F7).**
  ...
  **F7 default-FAIL:** `invalidation_set` RAISES on absent/malformed/partial `builds_against` (non-str
  ...
  (fix loop) — and (iii) writes a durable **`.kata/contract-invalidation.json`**
  `{contractId, oldHash, newHash, tasks:[{id, disposition}]}`. **F3 fail-closed cross-check:** the FINAL
  gate INDEPENDENTLY re-derives (from the frozen PLAN's `builds_against` + the git supersede trail)
  whether any supersede touched a contract; if so and the record is absent or does not fully-disposition
  `invalidation_set(C)`, the gate ⇒ **NEEDS_WORK** (IaC-gate independent-re-derivation pattern,
  `:537-540`) — soundness never rests on orchestrator compliance. **F5 restore-durable re-open:** a
  re-opened *integrated* task is recorded in **`.kata/invalidated.json`**; `kata_restore` subtracts it
  from the integrated set so a crash mid-invalidation re-dispatches it (the `Kata-Task:` trailer alone
  would mask it). Empty **well-formed** set ⇒ vacuous ⇒ today's `:507-509` path (BC).
...
- **M1-L8 — Contract immutability is machine-enforced between freeze and supersede (F4).**
  ...
  `surface_hash(contracts/<id>) != frozen pin` **without** a recorded supersede for `<id>` ⇒ NEEDS_WORK
  (reuses the `detect_drift`/`approval_verdict` fail-closed pattern). A provider filling stub *bodies*
  leaves the surface unchanged (no trip); any *interface* edit trips unless it went through supersede.
